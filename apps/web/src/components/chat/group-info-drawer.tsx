...
export function GroupInfoDrawer({ open, onClose, conversation }: Props) {
  ...
  const members = conversation?.members as Array<Record<string, unknown>> | undefined;
  const currentMember = members?.find((m) => m.userId === user?.id);
  const isAdmin = currentMember?.role === 'ADMIN';
  const existingMemberIds = new Set(members?.map((m) => m.userId as string) || []);
  ...
  return (
    <Drawer
      title={(conversation?.name as string) || 'Group Info'}
      open={open}
      onClose={onClose}
      width={320}
    >
        ...
        <div>
          ...
          <div className="space-y-2">
            {members?.map((member: Record<string, unknown>) => {
              const isOnline = !!onlineUsers[member.userId as string];
              return (
                <div key={member.id as string} className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className="relative">
                      <Avatar
                        size={32}
                        icon={<UserOutlined />}
                        src={(member.user as Record<string, unknown>)?.avatarUrl as string}
                      />
                    ...
                    <div>
                      <div className="text-sm flex items-center gap-1">
                        {(member.user as Record<string, unknown>)?.displayName as string || 'Unknown'}
                        {member.role === 'ADMIN' && (
                          <CrownFilled className="text-yellow-500 text-xs" />
                        )}
                        {(member.userId as string) === user?.id && (
                  ...
                  {isAdmin && (member.userId as string) !== user?.id && (
                    <Popconfirm
                      title="Remove this member?"
                      onConfirm={() => handleRemove(member.userId as string)}
                    >
                      <Button
                        type="text"
                        size="small"
