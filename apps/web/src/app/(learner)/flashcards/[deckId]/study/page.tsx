...
export default function StudyModePage() {
  ...
  useEffect(() => {
    startStudy.mutate(deckId, {
      onSuccess: (data) => { setSessionId(data.session.id); setCards(data.cards); },
    });
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8 select-none">
      ...
      <div className="flex items-center justify-between mb-6">
        <button onClick={() => router.push(`/flashcards/${deckId}`)} className="flex items-center gap-2 text-muted-foreground hover:text-foreground font-medium transition-colors cursor-pointer text-sm">
          ...
          <div className={slideClass} style={{ width: '100%', height: '100%' }}>
            <div className="flashcard-perspective flashcard-hover w-full h-full cursor-pointer" onClick={handleFlip}>
              <div className={`flashcard-inner relative w-full h-full ${isFlipped ? 'flipped' : ''}`}>
                ...
                {/* Back */}
                <div className="flashcard-face flashcard-back absolute inset-0 bg-emerald-50 rounded-2xl border-[2.5px] border-border-strong flex flex-col items-center justify-center p-6 sm:p-10 shadow-[4px_4px_0px_var(--shadow-brutal)]">
